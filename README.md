...
- **RAG 评估框架**：当前真实资料评测集为 162 个切片、210 条问答，支持 5 种模式对比、分组统计和失败分析。最近一次本地 `light-rerank` retrieval-only 结果为通过率 98.10%、Recall@8 94.56%、MRR@8 0.805；这不是线上准确率或生成质量指标。
...
| --- | ---: | ---: | ---: | ---: |
| `bm25-local` | 204/210（97.14%） | 93.93% | 0.784 | 约 15/23 ms |
| `light-rerank` | 206/210（98.10%） | 94.56% | 0.805 | 约 17/29 ms |
