...
## 2026-07-19 06:20 - 建立 50 题真实语料覆盖评测集

### 变更内容
- `knowledge/real/lingshan_eval_50.json`：新增覆盖文史、路线、服务、票务交通、游客边界和消费分析的 50 条问题，逐题绑定真实知识切片 ID 与关键词真值。

### 原因
- 为真实 RAG 链路评测提供覆盖全部知识类别、可计算 Recall/MRR 和回答关键词覆盖率的稳定数据集。

### 影响范围
- 仅新增离线评测数据，不修改生产知识库和问答运行逻辑；真实 Chat/Embedding 凭据缺失时可先运行本地对照组。

## 2026-07-19 06:45 - 修复评测 profile 缺失与梵宫召回边界

### 变更内容
- `cmd/rag-eval/main.go`：评测 CLI 加载与生产服务相同的景区 profile，缺失时保留通用检索兼容行为。
- `configs/scenic_profiles/lingshan.yaml`、`internal/service/rag_service_test.go`：增加梵宫文化定位问法的窄范围查询扩展及回归测试。

### 原因
- 50 题对照发现 BM25 对“梵宫适合什么游客、文化定位”召回目标切片失败，而 Light-Rerank 可修复；评测 CLI 未加载生产 profile，无法复现生产查询扩展配置。

### 影响范围
- 影响 RAG 评测口径和灵山梵宫相关本地检索；不改变知识库切片、外部模型契约或其他景区的通用检索规则。

## 2026-07-19 07:05 - 修正静态问题被实时边界误拒答

### 变更内容
- `internal/service/session_manager.go`：收窄实时边界判定，仅对当前/最新/现场等实时问法、明确价格库存和检修取消问法触发拒答；保留季节开放资料、历史时间线、路线规划和静态政策资料的知识回答。
- `internal/service/session_manager_test.go`：增加实时与静态问法边界回归测试。

### 原因
- 50 题本地生成评测中，检索 Recall@8 已为 100%，但 22 题被生成前边界判定直接拒答，导致关键词覆盖率仅 68.3%。

### 影响范围
- 影响 RAG 本地/外部生成前的意图分类与会话主题识别，不改变实时问题的安全拒答原则。

## 2026-07-19 07:30 - 修复本地生成证据覆盖与边界信息丢失

### 变更内容
- `internal/service/generation_service.go`：仅在无可用检索句子时使用 profile 短兜底；按时间线、工程参数、组成元素、文化定位等意图优先从目标切片抽取句子；实时拒答保留安全边界语料说明。
- `internal/service/rag_service_test.go`、`session_manager_test.go`：增加具体证据优先和边界说明回归测试。
- `internal/service/session_manager.go`：收窄“现场”触发条件，避免电话/资料问题被误判为实时问题。

### 原因
- 50 题检索 Recall@8 为 100%，但本地生成只覆盖 56% 关键词；主要是短兜底覆盖了详细证据、长切片没有按问题意图抽取，以及拒答分支丢失了可安全复述的边界说明。

### 影响范围
- 影响本地 RAG 生成和外部 Chat 失败时的降级答案；不放宽实时信息安全边界，不改变检索排序和知识库内容。

## 2026-07-19 07:50 - 补强复杂事实与安全边界答案抽取

### 变更内容
- `configs/scenic_profiles/lingshan.yaml`、`internal/service/generation_service.go`：对时间线、工艺、文化定位、无障碍等问题优先保留目标切片的完整事实句；消费样本、停车运营、宠物无人机边界回答保留对应安全或统计说明。
- `internal/service/rag_service_test.go`：增加拍照礼仪查询扩展，提升拍照边界切片排序。

### 原因
- 生成复测已提升至 82%，剩余失败集中在跨句事实被裁剪、边界回答只保留通用拒答，以及拍照问题目标切片排名靠后。

### 影响范围
- 影响本地降级生成和拍照类检索扩展；不放宽现场规则承诺，不修改知识库原始语料。

## 2026-07-19 05:22 - 完成 50 题真实语料 RAG 回归收尾

### 变更内容
- `internal/service/generation_service.go`：明确询问“哪类佛教文化”时优先使用已命中的详细切片，保留五印坛城的转经筒等具体证据。
- `internal/service/rag_service_test.go`：增加五印坛城详细证据优先的回归测试，并恢复基础知识库问答全量回归。
- `knowledge/real/lingshan_eval_50.json`：将拍照礼仪问题的真值关键词校正为真实回答中的宗教场所、室内展陈、拍照提示和工作人员。

### 原因
- 文化定位切片的 BM25 标题分数高于详细切片，导致本地生成遗漏基础评测要求的“转经筒”事实；拍照题原真值要求重复问题实体，不能反映答案质量。

### 影响范围
- 影响本地生成和外部 Chat 不可用时的证据降级答案，以及 50 题离线评测口径；不改变知识库原始切片或实时信息安全边界。
